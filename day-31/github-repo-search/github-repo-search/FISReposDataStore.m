...
-(void)searchForRepositoriesWithQuery:(NSString *)query completion:(void (^)(BOOL))completionBlock
{
    [FISGithubAPIClient searchRepositoriesWithQuery:query completion:^(NSArray *matchingRepositories) {
        for (NSDictionary *repoDictionary in matchingRepositories)
        {
            [self.repositories addObject:[FISGithubRepository repoFromDictionary:repoDictionary]];
        }
